...
/**
 * @author acmday
 * @date 2022/9/27 下午11:23
 *
 * 该扩展接口有3个触发点方法：
 * 1、predictBeanType：该触发点发生在postProcessBeforeInstantiation之前(一般不太需要扩展这个点)，这个方法用于预测Bean的类型，返回第一个预测成功的Class类型，如果不能预测返回null；
...
public class SmartInstantiationAwareBeanPostProcessorImpl implements SmartInstantiationAwareBeanPostProcessor {
    ...
    public Class<?> predictBeanType(Class<?> beanClass, String beanName) throws BeansException {
        log.info("act=[SmartInstantiationAwareBeanPostProcessor.predictBeanType] predictBeanType " + beanName);
        return beanClass;
    }

    @Override
    public Constructor<?>[] determineCandidateConstructors(Class<?> beanClass, String beanName) throws BeansException {
        log.info("act=[SmartInstantiationAwareBeanPostProcessor.predictBeanType] determineCandidateConstructors " + beanName);
        return null;
    }

    @Override
    public Object getEarlyBeanReference(Object bean, String beanName) throws BeansException {
        log.info("act=[SmartInstantiationAwareBeanPostProcessor.predictBeanType] getEarlyBeanReference " + beanName);
        return bean;
    }
}
